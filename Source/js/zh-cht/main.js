...
$(function() {
    $(window).scroll(function() {
        if ($(this).scrollTop() > 200) {
            $('.headerMain.hdBig').fadeIn().removeClass('hdBig').addClass('hdSmall');
        } else {
...
$(function() {
    ...
    $(window).scroll(function() {
      ...
      if ($('header').hasClass('headerSHOW')) {
            $('header').removeClass('headerHide');
            $('.breadcrumb_bg').removeClass('bcUp');
            $('.navMember').removeClass('bcUp');
      }else{
        if (t < p && p > 500) {
            //scroll down
            $('header').addClass('headerHide');
            $('.breadcrumb_bg').addClass('bcUp');
            $('.navMember').addClass('bcUp');
        } else {
            //scroll up
            // 添加判断条件
            $('header').removeClass('headerHide');
...
if ($(window).width() < 768) {
    ...
    $('.navMenuList, .nav_m').click(function(event) {
    ...
    $('.navMain_m').on('touchmove', function(event) {
        event.stopPropagation(); // 阻止事件冒泡，防止底层内容滚动
      });
...
//分類頁選單下拉: PC全館分類下拉 & M 第2層
$(".menu_dropDown").click(function() {
    if(!$(this).hasClass('menu_open')){
        $('.menu_dropDown').removeClass('menu_open');
        $('.menu_dropDown').siblings(".menu_in").hide();
        $(this).addClass('menu_open');
        $(this).siblings(".menu_in").toggle(200);
    }else{
        $(this).removeClass('menu_open');
        $(this).siblings(".menu_in").hide();
    }
    event.stopPropagation(); // 阻止事件冒泡，防止底层内容滚动
...
//分類頁選單下拉: M 第1層
$(".menu_dropDown_tit").click(function() {
    if ($(this).find(".icon_arrow").hasClass('arr_open')) {
        $(this).siblings().toggle(200);
        $(this).find(".icon_arrow").removeClass('arr_open');
        event.stopPropagation(); // 阻止事件冒泡，防止底层内容滚动
    } else {
        $(this).siblings().toggle(200);
        $(this).find(".icon_arrow").addClass('arr_open');
        event.stopPropagation(); // 阻止事件冒泡，防止底层内容滚动
...
$('.btnClose').click(function() {
    ...
    //M第1層關閉
    for (let i = 0; i < $('.menu_dropDown_tit').length; i++) {
        const s = $('.menu_dropDown_tit').eq(i);
        if(s.siblings('.menu_in').attr("style")==="display: block;"){
            s.siblings('.menu_in').removeAttr("style");
            s.find(".icon_arrow").removeClass('arr_open');
        }
    }
